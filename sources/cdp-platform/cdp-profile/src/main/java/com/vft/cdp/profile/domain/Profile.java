...
public class Profile implements ProfileModel {
    ...
    public String update(Traits newTraits, Platforms newPlatforms, Campaign newCampaign) {
        ...
        // CHECK AND MERGE
        if (newTraits != null && !traitsEquals(this.traits, newTraits)) {
        ...
        if (newPlatforms != null && !platformsEquals(this.platforms, newPlatforms)) {
            Platforms mergedPlatforms=mergePlatforms(this.platforms,newPlatforms);
            this.platforms = mergedPlatforms;
            hasChanges = true;
            log.debug("Platforms changed");
        }
        if (newCampaign != null && !campaignEquals(this.campaign, newCampaign)) {
            Campaign mergedCampaign=mergeCampaign(this.campaign,newCampaign);
            this.campaign = mergedCampaign;
            hasChanges = true;
    ...
    /*
     * Merge traits,Platforms,Campaign
     */
